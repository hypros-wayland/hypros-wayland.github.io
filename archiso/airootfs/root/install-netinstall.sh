...
# ---------------------------------------------------------------------------
# 2b. Download target system files from GitHub
# ---------------------------------------------------------------------------
GITHUB_BASE="https://raw.githubusercontent.com/plasmatvlinux/plasmatvlinux.github.io/main"
WGET="wget --no-check-certificate -q"

d --title "Downloading" --infobox "Downloading package list and system files from GitHub..." 6 65

$WGET "$GITHUB_BASE/archiso/packages.x86_64" -O /root/packages.x86_64 || \
    die "Failed to download packages.x86_64 from GitHub"
$WGET "$GITHUB_BASE/skel.tar.gz" -O /root/skel.tar.gz || \
    die "Failed to download skel.tar.gz from GitHub"

mkdir -p /root/skel
tar xzf /root/skel.tar.gz -C /root/skel || \
    die "Failed to extract skel.tar.gz"

d --title "Downloading" --infobox "Download complete — proceeding with installation." 5 50
sleep 1

# ---------------------------------------------------------------------------
# 2c. Virtualization detection (VMware -> open-vm-tools)
# ---------------------------------------------------------------------------
